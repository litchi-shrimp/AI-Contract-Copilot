...
from ..llm.prompt_builder import ContractModificationAgentPrompt
...
class ContractModificationAgent(BaseAgent):
    """合同修改Agent，负责管理和修改合同（整体和OutlineModificationAgent一样，暂时只是prompt的区别）"""
    ...
    def __init__(self, agent_name: str, contract_name: str = None, data: dict = None,match_data:list = None):
        """初始化合同修改Agent

        Args:
            agent_name: Agent名称
            contract_name: 合同文件名（文件模式）
            data: 合同数据字典（内存模式，优先于 contract_name 读文件）
        """
        super().__init__(agent_name, BASE_DIR / "configs" / "config_contract_modification.json")
        
        self.contract_path = BASE_DIR / "outputs" / contract_name if contract_name else None
        self.match_result_path = AGENT_DIR / "data" / "match_result.json"
        self.skill_list_path = AGENTS_DIR / "contract_modification_agent_skills.json"
        if not match_data:
            self.skill_list_path = AGENTS_DIR / "contract_modification_agent_wotem_skills.json"
        ...
        # 内存模式优先
        if data is not None:
            save_path = self.contract_path or (BASE_DIR / "outputs" / f"{agent_name}_contract.json")
            self.contract_manager = OutlineManager(data=data, match_data=match_data, outline_path=save_path)
        else:
            self.contract_manager = OutlineManager(outline_path = self.contract_path, match_result_path = self.match_result_path)
        ...
        self.tools = []
        self.prompt_builder = ContractModificationAgentPrompt(self.contract_manager, skill_descriptions=self.skill_descriptions)
        self.tool_manager = ToolManager(self.contract_manager, tools_json=self.tools)
    ...
    def interactive_run(self) -> tuple:
        ...
        Returns:
            (user_confirmed, contract_data): 
                user_confirmed: True=用户确认退出；False=异常/超时/取消
                contract_data: 最终合同数据字典
        """
        eb = self.event_bus
        welcome = "整体合同已为您生成完毕，如果你有任何问题或者进一步修改需求都可以与我交流，如果确认最后的合同，请输入确认~"
        if eb:
        ...
        while True:
            try:
                ...
                contract_snapshot = self.contract_manager.snapshot()
                ...
                if self._cancelled:
                    self._cancelled = False
                    self.contract_manager.restore(contract_snapshot)
                ...
                self.contract_manager.save_outline()
        ...
        contract_data = self.contract_manager.get_outline()
        if user_confirmed:
            modified_contract_text = self.contract_manager._format_outline()
        ...
        return (user_confirmed, contract_data)

if __name__ == "__main__":
    contract_modification_agent = ContractModificationAgent("modification_agent","initial_contract.json")
    contract_modification_agent.interactive_run()
